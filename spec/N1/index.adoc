...
For time-valued quantities, in addition to the definite duration UCUM units, FHIRPath defines calendar duration keywords, and corresponding UCUM unit annotations, for calendar duration units:

|===
|Calendar Duration |UCUM Annotation |Relationship to Definite Duration UCUM Unit
...
If the `unit` argument is provided, it must be the string representation of a UCUM code (or a FHIRPath calendar duration keyword that can be represented as a UCUM annotation), and is used to determine whether the input quantity can be converted to the given unit, according to the unit conversion rules specified by UCUM. If the input quantity can be converted, the result is true, otherwise, the result is false.
...
Implementations that do support units shall do so as specified by <<UCUM>>, augmented for calendar durations as defined in the toQuantity function.
