...
==== excluding(other: collection) : collection
...
e.g. Patient.children().excluding(name|birthDate) would return all the properties of the Patient except for the name and birthDate.
...
Returns true if there is a single focus, which is a FHIR primitive, and it has a primitive value (e.g. as opposed to not having a value and just having extensions). 

____

*Note to implementers*: The FHIR conceptual model talks about "primitives" as superclasses of the type Element that also have id and extensions. 
What this actually means is that a FHIR primitive is not a primitive in an implementation language. The introduction (section 2 above)
describes the navigation tree as if the the FHIR model applies - primitives are both primitives and elements with children. There is
an open issue about how this is represented in FHIRPath. The following are valid options:

* FHIR primitives have a value child, but they are automatically cast to FHIRPath primitives when comparisons are made
* The value of a FHIR primitive is a special value that is only available implicitly 
* The FHIR primitives are just another node, and the only way to access the real primitive value is by a .value

The FHIR specification contains many invariants that assume #3 is not the case, but it doesn't contain any invariants that 
cause a difference between #1 and #2 (by design). The important place is difference is whether the primitive value appears
in the set returned by children() or descendents().

Comment is welcome at http://wiki.hl7.org/index.php?title=FHIR_Specification_Feedback_(STU_3)
____
...
https://github.com/cqframework/clinical_quality_language/tree/v12
