...
Optional parameters to functions are enclosed in square brackets in the definition of a function. Note that the brackets are only used to indicate optionality in the signature, they are not part of the actual syntax of FluentPath.
...
* Years must be present (`-MM-DD` is not a valid dateTime in FluentPath)
...
Finally, FluentPath supports the notion of functions, which all take a collection of values as input and produce another collection as output and may take parameters. For example:
...
FluentPath functions and operators both propagate empty results, but the behavior is in general different when the argument to the function or operator expects a collection (e.g. `select()`, `where()` and `|` (union)) versus when the argument to the function or operator takes a single value as input (e.g. `+` and `substring()`).
...
Note that the bracket notation in function signatures indicates optional parameters, and is not part of the formal syntax of FluentPath.
...
== Lexical Elements
FluentPath defines the following lexical elements:
...
=== Whitespace
FluentPath defines _tab_, _space_, and _return_ as _whitespace_, meaning they are only used to separate other tokens within the language. Any number of whitespace characters can appear, and the language does not use whitespace for anything other than delimiting tokens.

=== Comments
FluentPath defines two styles of comments, _single-line_, and _multi-line_. A single-line comment consists of two forward slashes, followed by any text up to the end of the line:
...
=== Literals
Literals provide for the representation of values within FluentPath. The following types of literals are supported:
...
=== Symbols
Symbols provide structure to the language and allow symbolic invocation of common operators such as addition. FluentPath defines the following symbols:
...
=== Keywords
Keywords are tokens that are recognized by the parser and used to build the various language constructs. FluentPath defines the following keywords:
...
|`as`|`in`|`second`
|`contains`|`is`|`true`
|`day`|`millisecond`|`week`
|`div`|`minute`|`xor`
|`false`|`mod`|`year`
|===

In general, keywords within FluentPath are also considered _reserved_ words, meaning that it is illegal to use them as identifiers. If necessary, identifiers that clash with a reserved word can be quoted.

=== Identifiers
Identifiers are used as labels to allow expressions to reference elements such as model types and properties. FluentPath supports two types of identifiers, _simple_ and _quoted_.
...
FluentPath escape sequences for strings also work for quoted identifiers:
...
=== Case-Sensitivity
FluentPath is a case-sensitive language, meaning that case is considered when matching keywords in the language. However, because FluentPath can be used with different models, the case-sensitivity of type and property names is defined by each model.
...
FluentPath supports reflection to provide the ability for expressions to access type information describing the structure of values. The `type()` function returns the type information for each element of the input collection, using one of the following concrete subtypes of `TypeInfo`:
...
The formal syntax for FluentPath is specified as an http://www.antlr.org/[Antlr 4.0] grammar file (g4) and included in this specification at the following link:
...
As discussed in the section on case-sensitivity, each model used within FluentPath determines whether or not identifiers in the model are case-sensitive. This information is provided as part of the model information and tooling should respect the case-sensitive settings for each model.
...
- [[[XMLRegEx]]] Regular Expressions. https://www.w3.org/TR/xmlschema11-2/#regexs
...
[appendix]
== Use of FluentPath on HL7 Version 2 messages

FluentPath can be used against HL7 v2 messages. This UML diagram summarises the 
object model on which the FluentPath statements are written:
