...
==== | (union collections)
Merge the two collections into a single collection, eliminating any duplicate values (using equals (`=`)) to determine equality). Joining an empty collection to a non-empty collection will return the non-empty collection.
