...
* Unless specified otherwise by the underlying Object Model, the first item in a collection has index 0. Note that if the underlying model specifies that a collection is 1-based (the only reasonable alterantive to 0 based collections), _any collections genrated from operations on the 1 based list are 0 based_.
