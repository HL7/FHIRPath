...
Version: 1.1.0 Public Domain (http://creativecommons.org/publicdomain/zero/1.0/[Creative Commons 0])

FHIRPath is used by the link:http://hl7.org/fhir[FHIR Specification], as well as link:http://cql.hl7.org/03-developersguide.html#using-fhirpath[Clinical Quality Language].
...
If the input collection contains multiple items, the evaluation of the expression will end and singal an error to the calling environment.
...
* the item is an Integer, Decimal, Time, or DateTime
* the item is a Boolean
* the item is a Quantity
...
* the item in the input collection is an Integer, Decimal, Time, DateTime, or Quantity the output will contain its String representation
...
For `DateTime` and `Time` comparisons with partial values (e.g. date/times specified only to the day, or times specified only to the hour), the comparison returns empty (`{ }`), not `false`.
...
|DateTime
|The at-symbol (`@`) followed by a datetime
