...
Note: Section 7 - Aggregates and Section 10.2 - Reflection have not received significant implementation experience and are marked for Standard for Trial Use (STU). In addition, the appendices are included as additional documentation and are informative content.
...
** `Time`: must be exactly the same, respecting the timezone offset (though +00:00 = -00:00 = Z)
